...
static VOS_STATUS wlan_hdd_is_pno_allowed(hdd_adapter_t *pAdapter);
#endif
                                            ...
                                            int data_len)
{
    ...
    if (VOS_STATUS_SUCCESS != sme_SetLinkLayerStatsIndCB(
                               pHddCtx->hHal,
    ...
    if (VOS_STATUS_SUCCESS != sme_LLStatsSetReq( pHddCtx->hHal,
                                            ...
                                            int data_len)
{
    ...
    if (VOS_STATUS_SUCCESS  != sme_LLStatsGetReq( pHddCtx->hHal,
                                              ...
                                              int data_len)
{
    ...
    if (VOS_STATUS_SUCCESS == sme_LLStatsClearReq(pHddCtx->hHal,
                                         ...
                                         const void *data, int data_len)
{
   ...
   if (sme_IsFeatureSupportedByFW(RTT3)) {
    ...
    if (pHddCtx->cfg_ini->configPNOScanSupport &&
        (VOS_STATUS_SUCCESS == wlan_hdd_is_pno_allowed(pAdapter))) {
        ...
        int data_len)
{
    ...
    } else if (adapter->device_mode == VOS_P2P_GO_MODE ||
            adapter->device_mode == VOS_STA_SAP_MODE) {
...
void wlan_hdd_cfg80211_update_reg_info(struct wiphy *wiphy)
{
    ...
    if (VOS_STATUS_SUCCESS != sme_GetDefaultCountryCodeFrmNv(pHddCtx->hHal,
...
int wlan_hdd_cfg80211_update_apies(hdd_adapter_t *pHostapdAdapter)
{
    ...
    if ((pBeacon->proberesp_ies != NULL) && (pBeacon->proberesp_ies_len != 0))
    {
        ...
        */
        ...
        if (probe_rsp_ie_len[1] == 0 )
        {
            if ( VOS_STATUS_E_FAILURE == ccmCfgSetStr((WLAN_HDD_GET_CTX(pHostapdAdapter))->hHal,
    ...
    else
    {
        ...
        if ( VOS_STATUS_E_FAILURE == ccmCfgSetInt((WLAN_HDD_GET_CTX(pHostapdAdapter))->hHal,
...
VOS_STATUS wlan_hdd_validate_operation_channel(hdd_adapter_t *pAdapter,int channel)
{
    ...
    else
    {
        ...
        for (indx = 0; indx < num_ch; indx++)
        {
         ...
         if (indx >= num_ch)
         {
             if (VOS_P2P_GO_MODE == pAdapter->device_mode)
             {
                 eCsrBand band;
                 unsigned int freq;
                                 ...
                                 )
{
    ...
    else if ((pAdapter->device_mode == VOS_STA_SAP_MODE)
...
int wlan_hdd_disconnect( hdd_adapter_t *pAdapter, u16 reason )
{
    ...
    if((VOS_STATUS_CMD_NOT_QUEUED == status) &&
     ...
     */
    else if(VOS_STATUS_CMD_NOT_QUEUED == status)
    {
        hddLog(LOG1,
...
wait_for_disconnect:
    ...
    if (!ret && (VOS_STATUS_CMD_NOT_QUEUED != status))
    {
        hddLog(LOGE,
...
#endif
{
    ...
    if (pHostapdAdapter->device_mode == VOS_STA_SAP_MODE)
    {
        pIe = wlan_hdd_cfg80211_get_ie_ptr(pBeacon->tail, pBeacon->tail_len,
                                       WLAN_EID_COUNTRY);
    ...
    */
    ...
    if ( NULL != psmeConfig)
    {
        ...
        if (((pHostapdAdapter->device_mode == VOS_STA_SAP_MODE)
          || (pHostapdAdapter->device_mode == VOS_P2P_GO_MODE))
...
#endif
{
    ...
    if (VOS_STA_SAP_MODE == pAdapter->device_mode) {
        status = sme_RoamDelPMKIDfromCache(
       ...
       )
    {
        ...
        if ( VOS_STATUS_E_FAILURE == ccmCfgSetInt(pHddCtx->hHal,
                            WNI_CFG_ASSOC_RSP_ADDNIE_FLAG, 0, NULL,
                            eANI_BOOLEAN_FALSE) )
        {
            hddLog(LOGE,
                                  ...
                                  )
{
   ...
   if (!pAdapter)
    ...
    if (((pAdapter->device_mode == VOS_P2P_DEVICE) &&
                ...
                if (NULL != strstr(ndev->name,"p2p"))
                {
                   ...
                   mutex_unlock(&pHddCtx->tdls_lock);
                    ...
                    pAdapter->device_mode = (type == NL80211_IFTYPE_STATION) ?
                                VOS_P2P_DEVICE : VOS_P2P_CLIENT_MODE;
                }
                else
                {
                    pAdapter->device_mode = (type == NL80211_IFTYPE_STATION) ?
                                VOS_STA_MODE: VOS_P2P_CLIENT_MODE;
                }
                break;
            ...
            case NL80211_IFTYPE_P2P_GO:
            {
               ...
               if (NL80211_IFTYPE_AP == type)
                ...
                if(VOS_STATUS_E_FAILURE ==
                       hdd_disable_bmps_imps(pHddCtx, VOS_P2P_GO_MODE))
                {
                    //Fail to Exit BMPS
                    VOS_ASSERT(0);
                }
          ...
          bool update, tCsrStaParams *StaParams)
{
    ...
    if ( VOS_STATUS_SUCCESS != pAdapter->tdlsAddStaStatus)
    {
        VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
                                         ...
                                         struct station_parameters *params)
{
    ...
    else if ((pAdapter->device_mode == VOS_STA_MODE)
          || (pAdapter->device_mode == VOS_P2P_CLIENT_MODE)) {
#ifdef FEATURE_WLAN_TDLS
...
#endif
{
    ...
    if ((VOS_IBSS_MODE == pAdapter->device_mode) && !pairwise)
    {
        setKey.keyDirection = eSIR_TX_RX;
        /*Set the group key*/
        status = sme_RoamSetKey( WLAN_HDD_GET_HAL_CTX(pAdapter),
              ...
              (pAdapter->device_mode == VOS_P2P_CLIENT_MODE) )
    {
        ...
        /* The supplicant may attempt to set the PTK once pre-authentication
           ...
           request */
        vos_status = sme_FTUpdateKey( WLAN_HDD_GET_HAL_CTX(pAdapter), &setKey);
        if ( vos_status == VOS_STATUS_FT_PREAUTH_KEY_SUCCESS )
        {
           hddLog(VOS_TRACE_LEVEL_INFO_MED,
        ...
        else if ( vos_status == VOS_STATUS_FT_PREAUTH_KEY_FAILED )
        {
           hddLog(VOS_TRACE_LEVEL_ERROR,
 ...
 *
 */
static VOS_STATUS hdd_cfg80211_scan_done_callback(tHalHandle halHandle,
	       ...
	       bool is_p2p_scan, tCsrScanRequest *csr_scan)
{
	...
	if (hdd_is_wiphy_scan_random_support(wiphy)) {
		...
		hal_status = sme_SpoofMacAddrReq(hdd_ctx->hHal,
						 ...
						 false);
		if (hal_status != VOS_STATUS_SUCCESS) {
			hddLog(LOGE,
                            ...
                            struct cfg80211_scan_request *request)
{
    ...
    else if(VOS_P2P_GO_MODE == pAdapter->device_mode)
    {
        MTRACE(vos_trace(VOS_MODULE_ID_HDD,
        ...
        const u8 *bssid_hint, u8 operatingChannel)
{
    ...
    if (pRoamProfile)
    {
        ...
        if ( (VOS_IBSS_MODE == pAdapter->device_mode) && operatingChannel)
        {
            ...
            case DOT11F_EID_WPA:
                ...
                if (!memcmp(genie, WPA_OUI_TYPE, WPA_OUI_TYPE_SIZE) &&
                   ...
                   if (ccmCfgSetStr(WLAN_HDD_GET_HAL_CTX(pAdapter),
                       ...
                       NULL, eANI_BOOLEAN_FALSE)== VOS_STATUS_SUCCESS)
                   {
                ...
                } /* end of if (VOS_IBSS_MODE == pAdapter->device_mode) */
                break;
            case DOT11F_EID_RSN:
                if (eLen > (MAX_WPA_RSN_IE_LEN - 2)) {
                                         ...
                                         )
{
    ...
    if (ccmCfgSetInt(pHddCtx->hHal,
        ...
        eANI_BOOLEAN_FALSE) != VOS_STATUS_SUCCESS)
    {
        hddLog (VOS_TRACE_LEVEL_ERROR,
        ...
        int dbm)
{
    ...
    case NL80211_TX_POWER_LIMITED: /*limit TX power by the mBm parameter*/
       if( sme_SetMaxTxPower(hHal, bssid, selfMac, dbm) != VOS_STATUS_SUCCESS )
       {
...
static VOS_STATUS wlan_hdd_is_pno_allowed(hdd_adapter_t *pAdapter)
{
   ...
   while ((NULL != pAdapterNode) && (VOS_STATUS_SUCCESS == status))
   {
        ...
        if (((VOS_STA_MODE == pTempAdapter->device_mode)
          && (eConnectionState_NotConnected != pStaCtx->conn_info.connState))
          || (VOS_P2P_CLIENT_MODE == pTempAdapter->device_mode)
          || (VOS_P2P_GO_MODE == pTempAdapter->device_mode)
          || (VOS_STA_SAP_MODE == pTempAdapter->device_mode &&
          ...
          struct net_device *dev, struct cfg80211_sched_scan_request *request)
{
    ...
    if (VOS_STATUS_SUCCESS != wlan_hdd_is_pno_allowed(pAdapter))
    {
        VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_WARN,
...
static int __wlan_hdd_cfg80211_testmode(struct wiphy *wiphy, void *data, int len)
{
    ...
    switch (nla_get_u32(tb[WLAN_HDD_TM_ATTR_CMD]))
    {
        ...
        case WLAN_HDD_TM_CMD_WLAN_HB:
        {
            ...
            if (VOS_STATUS_SUCCESS != smeStatus)
            {
               VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_ERROR,
   ...
   struct net_device *dev, struct cfg80211_csa_settings *csa_params)
{
   ...
   if (VOS_STA_SAP_MODE != adapter->device_mode)
        return -ENOTSUPP;
